...
| --- | --- |
| Foundations | Stage 1 IN STUDY — F001-F005 bounded direct Dart/Flutter execution; F006 harness containment but socket verdict/root cause OPEN |
...
### M006 — offline-before-navigation cold start validated; exact Chromium identity recovered
**VALIDATION:** exact head `3eef4e22b993018ea6b1787df0b82a819e545cdc`, run `35473544016`, job `105978759910` completed successfully. The workflow used GitHub Actions Ubuntu 24.04 runner image `20260907.300.1`, Playwright `1.55.0`, and exact Chromium `140.0.7339.16` / Playwright build `v1187`. Earlier M006 regressions and the new cold-start oracle all succeeded.

**TRANSFER VALIDATION:** after an online warm session established a dedicated precached shell/payload and service-worker control, the browser process was closed. A second persistent browser context launched against the same profile and was set offline before navigation. Navigation to the cached shell succeeded under service-worker control, exact shell/payload oracles passed, and a never-cached request failed. This adds a distinct lifecycle class beyond same-session offline and online-after-restart testing.

**EVIDENCE LIMIT:** synthetic Studio worker, loopback origin and Playwright offline emulation. The local origin server remained running while browser networking was forced offline. This is not physical connectivity loss, storage eviction, Flutter-generated/LogMate-postprocessed service worker, deployed HTTPS, Safari/WebKit/iPadOS/EFB or production evidence.

Canonical: `research/mobile/M006_real_browser_service_worker_transfer.md`.

## Retained evidence
- F001-F005: bounded direct Dart/Flutter execution appropriate to current claims.
...
- **Mobile → Systems:** exact Chromium identity for current M006 evidence is now known (`140.0.7339.16`, Playwright build `v1187`). Future product acceptance must additionally bind LogMate build/post-build artifact, origin and active worker/controller identity.
...
Do not repeat generic Chromium same-session offline, restart/profile reuse, synthetic update-transition, or generic Chromium cold-start variants; arbitrary Flutter SDK matrices; unauthenticated LogMate clone attempts; F002 aliasing variants; F003 queue timing sweeps; F006 close-order variants; or S005 flag permutations. Resume S004 immediately when authorized exact-ref source acquisition is available. Prefer a materially stronger/different evidence class: Safari/iPadOS/EFB or Android/iOS execution, canonical LogMate PWA runtime, storage-eviction or physical/real-connectivity transfer, or F006 phase-isolated root cause only with a better harness.
