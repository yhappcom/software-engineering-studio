...
### M003
**IN STUDY — USER-DIALOG TRANSFER VALIDATED; REPEATED-DENIAL / USER_FIXED LIFECYCLE VALIDATION IN PROGRESS; BOUNDED KEYSTORE SEMANTIC REPLICATION.** Canonical: `research/mobile/M003_android_user_permission_dialog_transfer.md`, `research/mobile/M003_android_repeated_denial_user_fixed_lifecycle.md`.
...
**REPEATED-DENIAL VALIDATION IN PROGRESS:** Android's documented Android-11+ denial lifecycle distinguishes first denial (`USER_SET`) from repeated permanent denial (`USER_FIXED`) and suppresses subsequent system permission dialogs. Exact validation head `bab16cc993d72d1ca98f3c9f2aa2bf4a4bbf8aa4` extends the same semantic-control fixture to require first denial, second denial, `USER_FIXED`, third-request dialog suppression and denied callback/UI. Run `35599908037` is in progress; no PASS/TRANSFER VALIDATION/ROOT CAUSE is awarded before terminal evidence.
...
- **Quality:** permission access state and requestability/dialog state need independent oracles; preserve phase-level failure evidence.
...
- **Design Studio:** ordinary denial versus dialog-suppressed permanent denial can require different recovery/rationale/settings UX; no Design canonical file edited.
- **Web Manager / Marketing Manager:** no material dependency for this block.

## CHANGE WATCH / OPEN
- Repeated-denial run `35599908037` pending; do not infer documented behavior from source alone.
...
## Next work
Complete the current repeated-denial professional boundary from terminal run evidence. If it passes, record only bounded API-35 lifecycle transfer and return to Balance Loop; if it fails, use the named first-failed phase for reproduction/isolation rather than attributing platform cause from one failure.
