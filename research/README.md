...
- `M003` — runtime dangerous-permission grant/revoke transfer validated. Android Keystore process/tamper transfer now has bounded TRANSFER VALIDATION PASS at exact head `0ea436129fa150b578c777abf1ac6a6500b84a4e`, run `35538756528` attempt 1, job `106152447185`: AndroidKeyStore AES-256/GCM key observed `NONEXPORTABLE` through `SecretKey.encoded`, fresh-process ciphertext recovery succeeded, external ciphertext mutation produced `AUTH_FAIL`, and the job completed naturally. Same-head replication attempt 2 is pending; the earlier run failure remains an unexplained CONTRADICTION, not root cause. Hardware-backed/StrongBox, auth-bound keys, physical device, backup/restore, iOS and product transfer remain OPEN. Canonical: `research/mobile/M003_android_keystore_secure_storage_transfer.md`.
