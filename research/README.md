...
- `F003` — `research/foundations/F003_data_structures_algorithms_complexity.md`; run `35458320697`, job `105937470606` validates direct Dart 3.13.3 FIFO semantic equivalence for growable List front-removal versus `ListQueue.removeFirst()` under an independent checksum/sequence oracle. Dart API source supplies the representation/cost contract; CI timing is diagnostic only. Space cost, broader search/hash algorithms and product/platform performance transfer remain OPEN.
