...
Status: **IN STUDY — SOURCE/MODEL + FIRST EXECUTABLE COUNTEREXAMPLE/SHRINKING EVIDENCE**  
Date: 2026-09-17  
...
- Hypothesis 6.168.0 stateful-testing documentation checked 2026-09-17: rule-based state machines generate sequences of actions; invariants can be checked after rules; implementations can be compared with simplified models; failing executions can be reduced to short reproducing programs. https://hypothesis.readthedocs.io/en/latest/stateful.html
...
The reference model need not reproduce implementation architecture. Excessive structural similarity can create correlated defects. It should encode the smallest independent semantics needed by the oracle.
...
## EXECUTABLE VALIDATION — FIRST COUNTEREXAMPLE + SHRINKING BLOCK
Fixture: `research/quality/fixtures/Q004_model_property_sequence_shrinking.py`

### Test Evidence Contract
- **CLAIM:** generated state histories plus an independent version-aware oracle can expose arrival-order synchronization defects, and deletion-based reduction can preserve a smaller reproducer.
- **SPEC/PROPERTY:** terminal observable state from the deliberately naive arrival-order replica must equal a reference model in which the highest version wins and delete remains a semantic state.
- **TARGET:** stdlib-only bounded Python fixture; this is a testing-method model, not Dart/Flutter or a product implementation.
- **INPUT/STATE:** fixed seed `20260917`; up to 500 sequences; length 1–8; alphabet `update_v1`, `update_v2`, `delete_v3`.
- **ORACLE:** independently implemented version comparison in `model_terminal`; violation is `naive_terminal != model_terminal`.
- **ENVIRONMENT:** Python 3.13.5; execution date 2026-09-17. Dart and Flutter executables were separately rechecked and unavailable.
- **OBSERVATION:** the first generated case already violated the property: `delete_v3, update_v2, update_v2`; naive terminal state was `B`, model terminal state was `None`. Greedy deletion reduced it to `delete_v3, update_v2`, which remained a violation and satisfied the fixture's 1-minimal-under-single-command-deletion check.
- **VERDICT:** **VALIDATION — COUNTEREXAMPLE FOUND AND REDUCED.** The campaign detects the intended stale-update-after-newer-delete resurrection class in this bounded model.
- **FAILURE MODEL:** arrival-order replacement discards version/deletion ordering information and can resurrect state after a newer delete.
- **REPRODUCTION DATA:** fixture path above, seed `20260917`, constants `CASES=500`, `MAX_STEPS=8`.
- **EVIDENCE LIMIT:** one seed/campaign is counterexample-search evidence, not coverage proof; greedy deletion establishes only 1-minimality under this deletion operator; no real network/backend/process-death/Dart/Flutter/product behavior is established.

### CONTRADICTION
The prior run's infrastructure `GatewaySelectionError` prevented execution and therefore left Q004 executable evidence OPEN. A trustworthy Python execution facility is now available, so that specific infrastructure dependency is resolved for this fixture. It does **not** resolve the separate Dart/Flutter runtime dependency.

### Regression extraction
The reduced sequence `delete_v3 → update_v2` is now the canonical deterministic regression reproducer for this Q004 transfer case. This is useful because a generated campaign should not be the only future guard for a known defect class. The deterministic case is still a model-level regression, not a LogMate/MintTap regression test.
...
D006 → Q004: the stale-update/delete invariant supplies the reusable stateful failure class, and this run reproduced that class through generated histories and reduced it to `delete_v3 → update_v2`. Data-owned synchronization semantics remain canonical in D006; Q004 owns the generated-testing method, not product sync policy.

## RELATED DOMAIN CHECK
- **Foundations:** F003 complexity matters because state/action spaces grow combinatorially; F004/F005 supply concurrency/async ordering mechanisms. F001 direct Dart/Flutter execution remains OPEN after environment recheck.
...
## OPEN / VALIDATION
1. Add a deliberate version-aware-SUT regression mutant and confirm the campaign kills it, rather than relying only on the intentionally naive target.
2. Compare generated sampling with bounded exhaustive enumeration on a small state space to quantify missed-state risk.
3. Add preconditions/invalid-action handling and distinguish model bug, generator bug, oracle bug and SUT bug.
4. Transfer to a real Dart/Flutter test framework only when a trustworthy SDK is available; do not infer Dart behavior from Python.
5. Later evaluate a maintained PBT framework by exact version, shrink/reproduction behavior and project fit rather than adopting it from popularity.

## HANDOFFS
- **Quality → Data:** the deterministic `delete_v3 → update_v2` counterexample is suitable as a bounded regression oracle when D006 evolves, but Data must define delete/recreate/tombstone-GC semantics before expanding generation.
...
## Current judgment
Q004 now has first executable counterexample and reduction evidence. It remains **IN STUDY / NOT PASS** because mutation sensitivity, broader state-space/search comparison, Dart/Flutter transfer, and real failure/runtime evidence remain OPEN.